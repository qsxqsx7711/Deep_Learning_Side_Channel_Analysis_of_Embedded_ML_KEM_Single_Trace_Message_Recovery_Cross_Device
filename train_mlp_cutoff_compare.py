...
# 固定使用 Butterworth filter 階數 7 (order=7)
FILTER_ORDER   = 1

# 其餘超參數可照原本設定
OPT            = 'o3'
ALGO           = 'PQClean'
FT             = 'ChebyshevI' # butterworth
...
# ---------- 工具函式 ----------
def load_trace_by_cutoff(cutoff_mhz):
    """
    根據 cutoff 頻率 (MHz) 讀取濾波後的 trace.npy 與 label.npy。
    目錄結構：trace_train_{FT}_filter/cf{cutoff_mhz}M_order{FILTER_ORDER}/
    """
    base_folder = f'./trace_train_{FT}_filter'
    folder_name = f'cf{cutoff_mhz}K_order{FILTER_ORDER}'
...
def train_one_cutoff(cutoff_mhz):
    print(f"\n=== Training cutoff={cutoff_mhz} MHz (order={FILTER_ORDER}) ===")
    X, y, scaler = load_trace_by_cutoff(cutoff_mhz)
    Xt, Xv, yt, yv = split_data(X, y)

    # Save scaler
    scaler_dir = f'model/{PLATFORM}/train_mlp_cutoff_compare/cf{cutoff_mhz}M/scalers'
    ...
    # Callbacks：儲存驗證集最佳模型
    model_dir = f'model/{PLATFORM}/train_mlp_cutoff_compare/cf{cutoff_mhz}M'
    ...
    # Save history
    hist_dir = f'figure/train_mlp_cutoff_compare/{PLATFORM}/cf{cutoff_mhz}M'
...
if __name__ == "__main__":
    ...
    # 針對 1~7 MHz 的 cutoff 進行訓練
    for cf in CUTOFF_FREQS:
        acc, f1, hist = train_one_cutoff(cf)
        best_acc[cf]     = acc
        best_f1[cf]      = f1
        histories[cf]    = hist

    # Export summary CSV (index 為 cutoff 頻率)
    summary_df = pd.DataFrame({
        'best_val_accuracy': pd.Series(best_acc),
        'best_val_f1'      : pd.Series(best_f1)
    }, index=pd.Index(CUTOFF_FREQS, name='cutoff_MHz'))
    out_summary_path = f'figure/train_mlp_cutoff_compare/{PLATFORM}/summary_by_cutoff.csv'
    ...
    fig_dir = f'figure/train_mlp_cutoff_compare/{PLATFORM}/compare_summary'
    os.makedirs(fig_dir, exist_ok=True)

    # 1) Validation Accuracy
    plt.figure(figsize=(8, 6))
    for cf, hist in histories.items():
        plt.plot(
            range(1, len(hist['val_accuracy'])+1),
            hist['val_accuracy'],
            label=f'{cf} MHz'
        )
    plt.title('Validation Accuracy –  Cutoff 1~7 MHz', fontsize=16)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(fig_dir, 'val_acc_cutoff_all.png'), dpi=200)
    plt.close()
    ...
    plt.title('Validation F1-score –  Cutoff 1~7 MHz', fontsize=16)
    plt.rc('xtick', labelsize=12)
    plt.rc('ytick', labelsize=12)
    plt.xlabel('Epoch', fontsize=14)
    ...
    plt.legend(fontsize=10)
    plt.tight_layout()
    plt.savefig(os.path.join(fig_dir, 'val_f1_cutoff_all.png'), dpi=200)
    plt.close()
